  ...
  return (
    <div className="flex h-full items-center gap-1.5 overflow-hidden">
      {shown.map((skill) => (
        <span
          key={skill}
          className="max-w-[130px] shrink truncate rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600 ring-1 ring-inset ring-slate-200 dark:bg-slate-700/50 dark:text-slate-300 dark:ring-slate-600/50"
        >
          {skill}
        </span>
      ))}
